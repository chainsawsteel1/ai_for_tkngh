...
*/
	...
	kazutori: {
		...
		intro: minutes => `先生方、数取りゲームしましょう！\n0~100の**整数**の中で最も大きい数字を取った先生が勝ちです。他の先生方と被ったらだめですよ～\n制限時間は${minutes}分です。数字はこの投稿にリプライで送ってくださいね！`,

		finish: 'ゲームの結果発表です！',

		finishWithWinner: (user, name) => name ? `今回は${user}先生(${name})の勝ちです！またやりましょう♪` : `今回は${user}先生の勝ちです！またやりましょう♪`,
	 ...
	 */
	fortune: {
		cw: name => name ? `私が今日の${name}の運勢を占いました...` : '私が今日の先生の運勢を占いました...',
	},
	...
	server: {
		cpu: 'あれれ～？♡こんなのでサーバー負荷高くなってるの～？♡ざぁこ♡ざこ鯖♡スペック上げろ♡'
	},
	...
	noting: {
		notes: [
			...
			'Dockerコンテナから投稿してます',
			'今日はTKNGH本部に来てます！',
			'TKNGH本部は、地球のどこかにあります',
			'TKNGH本部には、サーバーとかいう機械があります',
			'しっぽはないですよ？',
			'ひゃっ…！\nネコミミ触られると、くすぐったいです',
			'抗逆コンパイル性って、なにかな？',
